...
class FN121TrawlFilter(FN121AttributeFilter):
    ...
    vessel__not = ValueInFilter(field_name="vessel__abbrev", exclude=True)
    vessel__null = django_filters.BooleanFilter(
        field_name="vessel__abbrev", lookup_expr="isnull"
    )
    ...
    vessel_direction__null = django_filters.BooleanFilter(
        field_name="vessel_direction", lookup_expr="isnull"
    )

    vessel_speed = django_filters.NumberFilter(
        field_name="vessel_speed", lookup_expr="exact"
    )
    vessel_speed__gte = django_filters.NumberFilter(
        field_name="vessel_speed", lookup_expr="gte"
    )
    vessel_speed__lte = django_filters.NumberFilter(
        field_name="vessel_speed", lookup_expr="lte"
    )
    vessel_speed__gt = django_filters.NumberFilter(
        field_name="vessel_speed", lookup_expr="gt"
    )
    vessel_speed__lt = django_filters.NumberFilter(
        field_name="vessel_speed", lookup_expr="lt"
    )
    ...
    warp__null = django_filters.BooleanFilter(field_name="warp", lookup_expr="isnull")
